...
test_p = '../data/contest_data/preliminary_test_a/word_per_line_preliminary_A.txt'
test_b = '../data/contest_data/preliminary_test_b/word_per_line_preliminary_B.txt'
pseudo_p = f'{data_path}/vote_pseudo_tag420.txt'
pseudo_testb_p = f'{data_path}/vote_testb_pseudo_tag420.txt'
unlabel_pseudo_p = f'{data_path}/vote_unlabel_pseudo_tag420.txt'
tokenizer = BertTokenizerFast.from_pretrained(bert_path)
tokenizer.add_special_tokens({'pad_token': '[PAD]'})

save_model_name = f'../data/best_model/{bert_type}-{head_type}-{batch_size}-{learning_rate}'
submit_name = f'../data/submission/{bert_type}-{head_type}-{batch_size}-{learning_rate}'
...
testb_data = load_data(test_b)
pseudo_data = load_data(pseudo_p)
pseudo_testb_data = load_data(pseudo_testb_p)
unlabel_pseudo_data = load_data(unlabel_pseudo_p)
train_data += pseudo_data[:7000]
train_data += pseudo_testb_data[:5000]
train_data += unlabel_pseudo_data
print(train_data[:3])
...
def train(data_loader, _model, loss_fn, _optimizer):
    ...
    for batch, data in enumerate(data_loader):
        ...
        if use_pgd:
            _model.zero_grad()
            pgd = PGD2(_model, emb_name='word_embeddings.', epsilon=1.0, alpha=0.3)
            pgd.backup_grad()
...
def test(data_loader, loss_fn, _model):
    ...
    test_acc = numerate / all_p
    test_recall = numerate / all_t
...
if __name__ == '__main__':
    ...
    print("Done!")
    model.load_state_dict(torch.load(save_model_name, map_location='cuda:0'))
    res = predict(testb_data)
