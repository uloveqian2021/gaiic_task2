...
def read_data(args, tokenizer: BertTokenizerFast) -> dict:
    train_path = os.path.join(args.pretrain_data_path, 'train.json')
    dev_path = os.path.join(args.pretrain_data_path, 'dev.json')
    unlabel_data = os.path.join(args.pretrain_unlabel_data_path, 'unlabeled_train_data.txt')
    # train_df = pd.read_csv(train_path, sep='\t')
    # dev_df = pd.read_csv(dev_path, sep='\t')
    train_data = read_json(train_path)
    dev_data = read_json(dev_path)
    inputs = defaultdict(list)
    for i, row in tqdm(train_data):
        content = row

        inputs_dict = tokenizer(content,
                                max_length=128,
                                truncation=True,
                                padding='max_length')
    ...
    for i, row in tqdm(dev_data):
        ...
        inputs['token_type_ids'].append(inputs_dict['token_type_ids'])
        inputs['attention_mask'].append(inputs_dict['attention_mask'])
    with open(unlabel_data, 'r', encoding='utf-8') as fr:
        for line in fr.readlines():
            inputs_dict = tokenizer(line.strip(),
                                    max_length=128,
                                    truncation=True,
                                    padding='max_length')
...
class DGDataCollator:
    def __init__(self, max_seq_len: int, tokenizer: BertTokenizerFast, mlm_probability=0.15):
...
def main():
    ...
    parser.add_argument('--pretrain_unlabel_data_path', type=str,
                        default='../data/contest_data/train_data')
    parser.add_argument('--pretrain_data_path', type=str,
                        default='../data/public_data')
    parser.add_argument('--pretrain_model_path', type=str,
                        default=f'../macbert_base')
    parser.add_argument('--data_cache_path', type=str,
                        default=f'../datasets/JDNER/user_data/pretrain.pkl')
    parser.add_argument('--vocab_path', type=str,
                        default=f'../data/pretrain_model/macbert_base/vocab.txt')
    parser.add_argument('--save_path', type=str,
                        default='../data/pretrain_model')
    parser.add_argument('--record_save_path', type=str,
                        default='../data/pretrain_model/pretrain_model_record')
